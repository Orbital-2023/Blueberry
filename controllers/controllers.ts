...
import {
  IMeetCode,
  MeetingCodeModel as MeetingCode,
} from '../models/MeetingCode.model'
...
function formatTime(timeStr: string): string {
  ...
  return time
    .toLocaleString('en-US', { hour: 'numeric', hour12: true, timeZone: 'UTC' })
    .toLowerCase()
    .toLowerCase()
    .replace(/\s/g, '')
}
...
function convertToDayArray(schedule: BusySchedule): Record<string, string[]> {
  ...
  for (const email in schedule) {
    ...
    for (const event of details.busy) {
      ...
      const dayOfWeek = new Date(event.start).toLocaleString('en-US', {
        weekday: 'short',
      })
...
export const getEvents = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(MagicCalculation(response.data.calendars as BusySchedule))
  } catch (e: any) {
    res.status(400).json({ e: e.mesage })
  }
}
...
export const passwordCheck = async (req: Request, res: Response) => {
  ...
  const check = await MeetingCode.find({
    roomId: roomId,
    roomPassword: roomPassword,
  })

  if (check.length != 0) {
    return res.status(200).json(await MeetingCode.findOne({ roomId: roomId }))
  }
